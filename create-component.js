...
const componentPath = path.join(__dirname, 'src', 'renderer', 'components', 'MediaGrid.tsx');
...
export const MediaGrid: React.FC<MediaGridProps> = ({
  ...
  const handleMenuAction = (action: 'open' | 'delete' | 'tags' | 'remove') => {
    ...
    switch (action) {
      ...
      case 'delete':
        ...
        break;
      case 'tags':
        onEditTags?.(targetIds.map(id => mediaList.find(m => m.id === id)).filter(Boolean) as MediaFile[]);
        break;
      case 'remove':
  ...
  const handleSizeChange = (newSize: number) => {
    const clampedSize = Math.max(MIN_ICON_SIZE, Math.min(MAX_ICON_SIZE, newSize));
    onIconSizeChange?.(clampedSize);
  };
            ...
            >
              ...
              <div className="w-14 h-10 bg-gray-800 rounded-lg flex-shrink-0 flex items-center justify-center overflow-hidden shadow-sm">
                {media.thumbnail ? (
                  ...
                  <img
                    src={`file://${encodeURI(media.path.replace(/\/g, '/'))}`}
                    alt={media.filename}
                    className="w-full h-full object-cover"
                  />
                ) : (
  ...
  return (
    <>
      <div
        ref={containerRef}
        className="media-grid-container flex-1 overflow-auto p-3 relative"
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        {/* 操作提示 */}
        {showTooltip && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 bg-[#1a1a1a]/70 text-[#e0e0e0] text-xs px-3 py-1.5 rounded-full backdrop-blur-sm z-10 pointer-events-none trans
