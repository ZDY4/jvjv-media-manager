...
export const useMediaActions = () => {
  ...
  const handleAddFolder = useCallback(async () => {
    ...
    try {
      ...
      if (newMedia && newMedia.length > 0) {
        ...
        setWatchedFolders([...new Set([...watchedFolders, ...addedFolders])]);
        window.showToast?.({ message: `已添加 ${newMedia.length} 个文件`, type: 'success' });
        loadMedia();
      } else {
  ...
  const handleRefreshFolders = useCallback(async () => {
    ...
    try {
      await window.electronAPI.rescanFolders(watchedFolders);
      setScanProgress(null);
      loadMedia();
  ...
  }, [loadMedia, selectedMediaIds, lastSelectedId, filteredMediaList, setSelectedMediaIds, setLastSelectedId]);
  ...
  const handleClearPlaylist = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSelectedMediaIds(new Set());
        setLastSelectedId(null);
        window.showToast?.({ message: result.message || '播放列表已清空', type: 'success' });
      } else {
        window.showToast?.({ message: result.error || '清空失败', type: 'error' });
      }
    } catch (error) {
      console.error('清空播放列表失败:', error);
      window.showToast?.({ message: '清空播放列表失败', type: 'error' });
    }
  }, [setMediaList, setSelectedMediaIds, setLastSelectedId]);

  const handleRemoveFromPlaylist = useCallback((mediaIds: string[]) => {
