...
export const DataDirSetting: React.FC<DataDirSettingProps> = ({ onClose, onDataDirChanged }) => {
  ...
  const loadCurrentDir = async () => {
    try {
      ...
      window.showToast?.({ message: '加载目录失败', type: 'error' });
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-[#1a1a1a]/80 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg w-[600px] p-6">
        <h2 className="text-[#e0e0e0] text-xl mb-4">设置数据存储目录</h2>
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={onClose} className="text-gray-400 hover:text-[#e0e0e0] px-4 py-2">
            取消
          </button>
          <button
            onClick={handleSave}
            className="bg-blue-600 hover:bg-blue-700 text-[#e0e0e0] px-4 py-2 rounded"
          >
            保存
          </button>
        </div>
